 let data = {
        ...
        function loadStateFromLocalStorage() {
            const savedState = localStorage.getItem('dailyos_data');
            if (savedState) {
                data = JSON.parse(savedState);
            } else {
                // Initialize with some default accounts if none exist
                data.accounts = [
                    { id: 1, name: "Main Bank Account", type: "Bank", balance: 0, transactions: [] },
                    { id: 2, name: "Digital Wallet", type: "Wallet", balance: 0, transactions: [] }
                ];
            }
        }
        ...
        function saveStateToLocalStorage() {
            localStorage.setItem('dailyos_data', JSON.stringify(data));
        }

        // --- MAIN REFRESH FUNCTION ---
        function refreshUI() {
            // Flatten transactions for easy filtering
            const allTransactions = data.accounts.flatMap(acc => 
                acc.transactions.map(t => ({...t, accountName: acc.name, accountId: acc.id}))
            );
            
            // Re-populate and render all UI components
            populateAccountSelector(allTransactions);
            populateCategoryFilter(allTransactions);
            updateDashboardSummary(allTransactions);
            renderAccountsList(allTransactions);
            handleFilterChange();
            feather.replace();
        ...
        function setupEventListeners() {
            ...
            dateRangeFilterAnalytics.addEventListener('change', updateAllAnalytics);
            categoryFilterAnalytics.addEventListener('change', updateAllAnalytics);
            transactionHistoryFilter.addEventListener('change', updateAllAnalytics);
        ...
        function populateAccountSelector(allTransactions) {
        ...
        function populateCategoryFilter(allTransactions) {
            ...
            // Also populate analytics category filter
        ...
        function updateDashboardSummary(allTransactions) {
        ...
        function renderAccountsList(allTransactions) {
        ...
        function handleFilterChange() {
            ...
            // Re-filter by category after type filter
            if(filters.category) {
        ...
        function handleAddEditTransaction(e) {
            e.preventDefault();
            const transactionId = document.getElementById('transaction-id').value ? parseInt(document.getElementById('transaction-id').value) : null;
            ...
            // Revert old transaction and update account balances if editing
            if (oldTransaction) {
            ...
            if (oldTransaction && oldAccount.id === accountId) {
                Object.assign(oldTransaction, formData);
            } else {
                targetAccount.transactions.push(formData);
            }
        ...
        function openTransactionModal(transactionId = null) {
            ...
            if (transactionId) {
                ...
                const allTransactions = data.accounts.flatMap(acc => acc.transactions.map(t => ({...t, accountId: acc.id})));
        ...
        function renderAnalyticsCharts(monthlyData, categoryData, dailyData) {
            ...
            analyticsChartInstances.monthly = new Chart(monthlyCanvas, {
                type: 'bar',
                data: {
                ...
                options: {
                    ...
                    scales: { y: { beginAtZero: true, grid: { color: 'rgba(255, 255, 255, 0.1)' }, ticks: { color: 'white' } }, x: { grid: { color: 'rgba(255, 255, 255, 0.1)' }, ticks: { color: 'white' } } },
            ...
            analyticsChartInstances.category = new Chart(categoryCanvas, {
                type: 'doughnut',
                data: {
            ...
            analyticsChartInstances.daily = new Chart(dailyCanvas, {
                type: 'line',
                data: {
        ...
        // --- MODAL EDITING FUNCTIONS FOR ANALYTICS ---
        function openEditMonthlySpendingModal() {
            monthlyInputsContainer.innerHTML = '';
            const monthlyData = generateMonthlyData(data.accounts.flatMap(acc => acc.transactions.filter(t => t.type === 'debit')));
        ...
        function openEditCategorySpendingModal() {
            categoryInputsContainer.innerHTML = '';
            const categoryData = generateCategoryData(data.accounts.flatMap(acc => acc.transactions.filter(t => t.type === 'debit')));
        ...
        function openEditDailySpendingModal() {
            dailyInputsContainer.innerHTML = '';
            const dailyData = generateDailyData(data.accounts.flatMap(acc => acc.transactions.filter(t => t.type === 'debit')));
            const today = new Date();
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            setupEventListeners();
            refreshUI();
            showPage('transactions'); // Initial page load
        });
